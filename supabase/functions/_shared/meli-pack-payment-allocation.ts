...
export function allocatePackPayments(
  ...
  // 1) Strongest evidence: the order snapshot explicitly contains payment_id.
  ...
  // 2) Secondary validation: unique exact amount match.
  for (const payment of payments) {
  ...
  // 3) Prorate only a genuine single global pack payment.
