-- Fix enum/text comparisons in Documentos performance RPCs.
-- PostgreSQL does not define channel_type = text or document_type = text.
-- Keep RPC inputs as text for PostgREST/frontend ergonomics and cast enum columns.
...
begin
  ...
  with base as (
    ...
    where (p_channel = 'todos' or b.resolved_channel = p_channel)
      and (p_doc_type = 'todos' or b.document_type::text = p_doc_type)
      and (
        p_status = 'todos'
...
begin
  ...
  with docs as (
    ...
    where o.organization_id=v_org
      ...
      and (p_channel='todos' or o.channel::text=p_channel)
  ), sales_stats as (
    select
      count(*)::int sales,
