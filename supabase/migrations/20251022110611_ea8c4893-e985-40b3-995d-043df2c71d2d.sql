-- Create enum for settlement item types if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'settlement_item_type') THEN
    CREATE TYPE settlement_item_type AS ENUM ('SALE', 'REFUND', 'FEE', 'SHIPPING', 'ADJUSTMENT', 'CHARGEBACK');
  END IF;
END $$;

-- Create enum for tax document status if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tax_document_status') THEN
    CREATE TYPE tax_document_status AS ENUM ('issued', 'voided', 'pending');
  END IF;
END $$;

-- Create settlement_items table if not exists
...
DROP INDEX IF EXISTS settlement_items_channel_payment_unique;
...
DROP POLICY IF EXISTS "Users can view their own settlement items" ON settlement_items;
DROP POLICY IF EXISTS "Users can insert their own settlement items" ON settlement_items;
DROP POLICY IF EXISTS "Users can update their own settlement items" ON settlement_items;
...
ALTER TABLE bank_movements ADD COLUMN IF NOT EXISTS settlement_id UUID REFERENCES settlements(id);
...
DROP INDEX IF EXISTS tax_documents_external_unique;
CREATE UNIQUE INDEX tax_documents_external_unique 
...
DROP INDEX IF EXISTS tax_documents_external_order_id_idx;
CREATE INDEX tax_documents_external_order_id_idx 
ON tax_documents(external_order_id) 
WHERE external_order_id IS NOT NULL;

-- Add unique constraints to orders for idempotency
DROP INDEX IF EXISTS orders_channel_order_id_unique;
CREATE UNIQUE INDEX orders_channel_order_id_unique 
...
DROP INDEX IF EXISTS settlements_channel_external_id_unique;
CREATE UNIQUE INDEX settlements_channel_external_id_unique 
...
DROP TRIGGER IF EXISTS update_settlement_items_updated_at ON settlement_items;
