-- Update get_pending_sales to exclude cancelled orders
CREATE OR REPLACE FUNCTION public.get_pending_sales(p_limit integer DEFAULT 25, p_offset integer DEFAULT 0, p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_min_amount numeric DEFAULT NULL::numeric, p_marketplace text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, marketplace text, external_sale_id text, order_date timestamp with time zone, gross_amount numeric, customer_name text, product_title text, shipping_mode text, payment_method text, payment_method_type text, payment_method_brand text, installments integer, money_release_date timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Update get_pending_sales_stats to exclude cancelled orders
CREATE OR REPLACE FUNCTION public.get_pending_sales_stats(p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_min_amount numeric DEFAULT NULL::numeric, p_marketplace text DEFAULT NULL::text)
