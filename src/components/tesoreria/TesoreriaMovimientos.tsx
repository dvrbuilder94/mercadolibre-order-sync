...
function paymentStatus(p: TesoreriaPayment) {
  if (p.matchState === "orphan") {
    return { label: "Sin venta", className: "bg-red-50 text-red-700" };
  }
  if (p.matchState === "partial") {
    return { label: "Parcial", className: "bg-amber-50 text-amber-700" };
  }
  if (p.docsOk !== p.sales.length) {
    return { label: "Sin DTE", className: "bg-amber-50 text-amber-700" };
  }
  return { label: "Completo", className: "bg-emerald-50 text-emerald-700" };
}
...
export function TesoreriaMovimientos({ payments, initialMatchFilter = "all", onOpenOrder }: Props) {
  ...
  const channels = useMemo(() => {
    ...
    payments.forEach((p) => p.channels.forEach((c) => values.add(c)));
  ...
  // Sólo contexto visual: no altera ni reinterpreta payment_sales.
  const packStats = useMemo(() => {
    ...
    for (const payment of payments) {
      for (const sale of payment.sales) {
        ...
        const current = stats.get(sale.packId) || {
  ...
  const exportCsv = () => {
    const headers = [
      ...
      "documentos", "estado", "pack_ids", "neto_asignado",
    ];
  ...
  const renderDocument = (number: string, url: string | null) =>
    url ? (
      <a
        href={url}
        target="_blank"
      ...
      >
        {number}<ExternalLink className="h-3 w-3" />
      </a>
    ) : <span>{number}</span>;
  ...
  const renderCell = (column: ColumnKey, p: TesoreriaPayment) => {
    ...
    const multiOrderPack = !!stats && stats.sales.size > 1;
    ...
    switch (column) {
          ...
          <div>
            ...
            {p.methodBrand && (
              <div className="text-[10px] text-slate-400 uppercase">
                {p.methodBrand}{p.installments ? ` · ${p.installments}x` : ""}
              </div>
            )}
          </div>
        );
        ...
        return (
          <div className="flex flex-col gap-1">
            {p.channels.length === 0 ? <span className="text-slate-300">—</span> : p.channels.map((ch) => (
              <span
                key={ch}
                className={`text-[10px] px-1.5 py-0.5 rounded font-medium w-fit ${CHANNEL_COLOR[ch] || "bg-slate-100 text-slate-600"}`}
              >
                {channelLabel(ch)}
              </span>
            ))}
      ...
      case "bruto":
        return <span className="tabular-nums">{clp(p.gross)}</span>;
      case "comision":
        return <span className="tabular-nums text-slate-500">{p.fees ? `-${clp(p.fees)}` : "—"}</span>;
      case "otras":
        return <span className="tabular-nums text-slate-500">{p.otherDeductions ? `-${clp(p.otherDeductions)}` : "—"}</span>;
      case "neto":
        return <span className="tabular-nums font-semibold text-slate-900">{clp(p.net)}</span>;
      case "liberacion":
        return p.releaseDate ? (
          <span className={p.liberado ? "text-emerald-700" : "text-slate-700"}>
            ...
            {!p.exactRelease && <span title="Fecha no confirmada para todas las ventas" className="text-slate-400"> ≈</span>}
            <span className="block text-[10px] text-slate-400">
              {p.liberado ? "Liberado" : "Pendiente"}{!p.exactRelease ? " · estim." : ""}
            </span>
          </span>
        ) : <span className="text-slate-300">—</span>;
      case "ventas":
        return <span className="text-xs text-slate-600">{p.sales.length || 0}</span>;
      case "doc":
        if (p.documents.length === 0) return <span className="text-slate-300">—</span>;
        return (
          <span className="inline-flex items-center gap-1 text-xs">
            {renderDocument(p.documents[0].number, p.documents[0].url)}
            {p.documents.length > 1 && <span className="text-slate-400">+{p.documents.length - 1}</span>}
          </span>
        );
      case "estado":
        return (
          <span className={`text-[11px] px-1.5 py-0.5 rounded font-medium whitespace-nowrap ${status.className}`}>
            {status.label}
          </span>
        );
    }
      ...
      <div className="bg-white border rounded-xl p-3 flex flex-wrap items-center gap-2 relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Payment ID, orden, pack, documento…"
        ...
        <Select
          label="Estado"
          value={matchFilter}
          onChange={(v) => { setMatchFilter(v as MatchFilter); setPage(0); }}
          options={[["all", "Todos"], ["matched", "Asignados"], ["partial", "Parciales"], ["orphan", "Sin venta"]]}
        />
        <Select
          label="Pasarela"
          value={provider}
        ...
        {columnsOpen && (
          <div className="absolute right-3 top-12 z-20 w-72 bg-white border rounded-lg shadow-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-slate-700">Columnas</span>
              <button onClick={() => setColumnsOpen(false)} className="text-slate-400 hover:text-slate-700">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <button
              onClick={() => {
                persistVisible(DEFAULT_VISIBLE);
                persistOrder(COLUMNS.map((c) => c.key));
              }}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {slice.length === 0 && (
                <tr>
                  <td colSpan={orderedColumns.length + 1} className="px-3 py-12 text-center text-slate-400">
              ...
              {slice.map((p) => {
                ...
                const primaryPack = p.packIds[0] || null;
                const stats = primaryPack ? packStats.get(primaryPack) : null;
                const packTotal = stats
                  ? Array.from(stats.sales.values()).reduce((sum, amount) => sum + amount, 0)
                  : 0;
                const multiOrderPack = !!stats && stats.sales.size > 1;
                const assignedOk = Math.abs(p.allocatedSum - p.net) <= Math.max(Math.abs(p.net) * 0.02, 100);
                      ...
                      <tr className="border-b bg-slate-50/40">
                        ...
                        <td colSpan={orderedColumns.length} className="px-4 py-3">
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 mb-3">
                            {multiOrderPack && (
                              <span>
                                <b className="text-slate-700">Pack {primaryPack}</b>
                                {` · ${stats.sales.size} órdenes · ${clp(packTotal)} · ${stats.payments.size} pagos`}
                              </span>
                            )}
                            <span>Bruto <b className="text-slate-800 tabular-nums">{clp(p.gross)}</b></span>
                            <span>Comisión <b className="text-slate-800 tabular-nums">-{clp(p.fees)}</b></span>
                            {p.otherDeductions > 0 && (
                              <span>Otras deducciones <b className="text-slate-800 tabular-nums">-{clp(p.otherDeductions)}</b></span>
                            )}
                            <span>Neto <b className="text-slate-900 tabular-nums">{clp(p.net)}</b></span>
                            <span className={assignedOk ? "text-emerald-700" : "text-amber-700"}>
                              Neto asignado {clp(p.allocatedSum)} {assignedOk ? "✓" : "· parcial"}
                            </span>
                          </div>

                          {p.sales.length === 0 ? (
                            <p className="text-xs text-slate-500">Este payment no tiene ventas vinculadas.</p>
                          ) : (
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="text-left text-[10px] uppercase text-slate-400 border-b">
                                  <th className="py-1.5 pr-3">Orden</th>
                                  <th className="py-1.5 pr-3">Cliente / producto</th>
                                  <th className="py-1.5 px-3 text-right">Venta bruta</th>
                                  <th className="py-1.5 px-3 text-right">Neto asignado</th>
                                  <th className="py-1.5 pl-3">Documento</th>
                                </tr>
                              </thead>
                              <tbody>
                                {p.sales.map((sale) => (
                                  <tr key={sale.id} className="border-b last:border-0">
                                    <td className="py-2 pr-3">
                                      <button onClick={() => onOpenOrder(sale.id)} className="font-mono text-blue-600 hover:underline">
                                        {sale.orderId}
                                      </button>
                                    </td>
                                    <td className="py-2 pr-3 max-w-[360px]">
                                      <div className="text-slate-700 truncate">{sale.customer || "—"}</div>
                                      <div className="text-[10px] text-slate-400 truncate">{sale.title || "—"}</div>
                                    </td>
                                    <td className="py-2 px-3 text-right tabular-nums text-slate-700">{clp(sale.gross)}</td>
                                    <td className="py-2 px-3 text-right tabular-nums text-slate-700">{clp(sale.allocated)}</td>
                                    <td className="py-2 pl-3">
                                      {sale.documents.length === 0
                                        ? <span className="text-slate-400">Sin DTE</span>
                                        : (
                                          <span className="inline-flex items-center gap-1">
                                            {renderDocument(sale.documents[0].number, sale.documents[0].url)}
                                            {sale.documents.length > 1 && <span className="text-slate-400">+{sale.documents.length - 1}</span>}
                                          </span>
                                        )}
                                    </td>
                                  </tr>
  ...
  return (
    <label className="text-[11px] text-slate-400 flex items-center gap-1">
      ...
      <select
        ...
        className="text-xs text-slate-700 border rounded-md px-2 py-1.5 bg-white"
      >
