...
import {
  ...
  Wrench, Landmark, FileText, Undo2, MessageSquare, Database, UserRound,
} from "lucide-react";
...
export function Nav() {
  ...
  useEffect(() => { setAdv(localStorage.getItem(ADV_KEY) === "1"); }, []);
  const toggleAdv = () => {
    const next = !adv;
    setAdv(next);
  ...
  const renderLink = ({ to, label, icon: Icon }: typeof primary[number]) => (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive ? "bg-slate-100 font-medium text-slate-900" : "text-slate-500 hover:text-slate-900 hover:bg-slate-50",
      )}
    >
    ...
    <>
      <aside className="w-52 min-h-screen border-r bg-white flex flex-col py-6 px-3 shrink-0">
        <div className="px-3 mb-1">
          ...
          <p className="text-[10px] uppercase tracking-wider text-slate-400 mt-1">Multi-marketplace Ops</p>
        </div>
        ...
        <nav className="flex flex-col gap-1 flex-1 mt-6">
          ...
          {adv && (
            <>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 px-3 mt-5 mb-1">Avanzado</p>
              {advanced.map(renderLink)}
            </>
          )}
        </nav>

        <button onClick={toggleAdv} className={cn(
          ...
          <Wrench className="h-3.5 w-3.5" />Modo avanzado {adv ? "· on" : "· off"}
        </button>

        <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 rounded-md text-sm text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-colors">
